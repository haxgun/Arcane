...
async def cmd_eight_ball(self, ctx) -> None:
...
@cooldown(rate=1, per=5, bucket=commands.Bucket.member)
@command(name='accountage', aliases=['age'])
async def cmd_accountage(self, ctx) -> None:
...
@cooldown(rate=1, per=5, bucket=commands.Bucket.member)
@command(name='uptime')
async def cmd_uptime(self, ctx) -> None:
...
@cooldown(rate=1, per=5, bucket=commands.Bucket.member)
@command(name='followsince')
async def cmd_followsince(self, ctx) -> None:
...
async def cmd_followage(self, ctx) -> None:
    target_user = ctx.author.name
    channel_name = ctx.channel.name
